...
public class SearchRequest extends ActionRequest implements IndicesRequest.Replaceable {
    ...
    private final ConcurrentHashMap<Version, BytesReference> cachedSerializedSourceByVersion = new ConcurrentHashMap<>();
    ...
    public SearchRequest source(SearchSourceBuilder sourceBuilder) {
        ...
        // Invalidate any previously cached serialized source since content changed
        this.cachedSerializedSourceByVersion.clear();
        return this;
    }
     ...
     * This method caches the serialized bytes and returns them. Subsequent calls return the cached value.
     *
     ...
     */
    public BytesReference getOrCreateSerializedSource(Version version) throws IOException {
        if (source == null) {
            return null;
        }
        BytesReference cached = cachedSerializedSourceByVersion.get(version);
        if (cached != null) {
            return cached;
        }
        BytesStreamOutput out = new BytesStreamOutput();
        ...
        BytesReference previous = cachedSerializedSourceByVersion.putIfAbsent(version, serialized);
